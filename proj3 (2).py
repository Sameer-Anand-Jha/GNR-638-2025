...
DATA_PATH = '/home/guest1/abhishek/neha/data'
...
CATEGORIES = ['agricultural', 'airplane', 'baseballdiamond', 'beach', 'buildings',
              ...
              'harbor', 'intersection', 'mediumresidential', 'mobilehomepark', 'overpass','parkinglot','river','runway','sparseresidential','storagetanks','tenniscourt']

CATE2ID = {v: k for k, v in enumerate(CATEGORIES)}

ABBR_CATEGORIES = ['agr', 'air', 'bas', 'bea', 'bui', 'cha', 'den',
                   'for', 'fre', 'gol', 'har', 'int', 'med', 'mob', 'ove','par','riv','run','spa','sto','ten']
...
def main():
    ...
    train_image_paths, val_image_paths, train_labels, val_labels = \
        get_image_paths(DATA_PATH, CATEGORIES, NUM_TRAIN_PER_CAT,NUM_VAL_PER_CAT,NUM_TEST_PER_CAT)
    ...
    if FEATURE == 'tiny_image':
        ...
        if os.path.isfile('vocab50.pkl') is False:
            ...
            with open('vocab50.pkl', 'wb') as handle:
        ...
        if os.path.isfile('val_image_feats_50.pkl') is False:
            val_image_feats  = get_bags_of_sifts(val_image_paths);
            with open('val_image_feats_50.pkl', 'wb') as handle:
                pickle.dump(val_image_feats, handle, protocol=pickle.HIGHEST_PROTOCOL)
        else:
            with open('val_image_feats_50.pkl', 'rb') as handle:
    ...
    build_confusion_mtx(val_labels_ids, predicted_categories_ids, ABBR_CATEGORIES)
    visualize(CATEGORIES, val_image_paths, val_labels_ids, predicted_categories_ids, train_image_paths, train_labels_ids)

def build_confusion_mtx(val_labels_ids, predicted_categories, abbr_categories):
    # Compute confusion matrix
    cm = confusion_matrix(val_labels_ids, predicted_categories)
    ...
    plt.savefig("Confusion_matrix_" + FEATURE + "_" + CLASSIFIER + "_50.png")
